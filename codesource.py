...
working_url = "http://books.toscrape.com/catalogue/category/books/fantasy_19/index.html"
...
BASE_IMG_LINK = "http://books.toscrape.com/"
...
category_requests = requests.get(working_url)

if category_requests.ok:

    category_soup = BeautifulSoup(category_requests.text, "html.parser")
...
with open("results/{}.csv".format(category_name), "w") as file:
    ...
    for book_page_url in books_page_url_list:
        ...
        if response.ok:
            ...
            img = img["src"].replace("../../", BASE_IMG_LINK)
