...
SPACE_DELETION_EXPRESSION = re.compile(r" ")

RESULTS_NUMBER_EXTRACT_EXPRESSION = re.compile(r"\d+")\
    # regex used to extract the number of books in one category
...
BASE_IMG_URL = "http://books.toscrape.com/"
...
if main_request.ok:
    ...
    category_url_list = []
    category_name_list = []
    category_name_extraction = main_soup.find("aside")
    category_name_extraction = category_name_extraction.findAll("ul")
    category_name_extraction = category_name_extraction[1]
    category_name_extraction = category_name_extraction.findAll("li")
    for category_name in category_name_extraction:
        category_name = category_name.get_text().strip()
        category_name = SPACE_DELETION_EXPRESSION.sub("-", category_name)
        category_name_list.append(category_name)

    for position, category_name in enumerate(category_name_list):
        category_url = "{}{}_{}/index.html".format(BASE_CATEGORY_URL, category_name.lower(), position+2)
        ...
        if category_request.ok:
            ...
            while page_counter != page_number:
                if page_counter == 0:
                    ...
                    books_page_url_list.extend(book_page_url_extractor(iter_soup_category, BASE_BOOK_URL))
            ...
            with open("results/{}.csv".format(category_name), "w") as file:
                ...
                for book_page_url in books_page_url_list:
                    ...
                    if book_request.ok:
                        ...
                        img = img["src"].replace("../../", BASE_IMG_URL)
